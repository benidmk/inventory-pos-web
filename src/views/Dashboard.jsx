...
export default function Dashboard({ products = [], salesRecent = [] }) {
  ...
  // ====== NEW: total penjualan keseluruhan (all-time) ======
  const [totalAll, setTotalAll] = useState(0);
  const [loadingTotalAll, setLoadingTotalAll] = useState(false);
  const [errorTotalAll, setErrorTotalAll] = useState("");

  useEffect(() => {
    (async () => {
      try {
        setLoadingTotalAll(true);
        setErrorTotalAll("");
        // tanpa from/to => all-time
        const r = await api.reports.totalSales();
        setTotalAll(Number(r?.total || 0));
      } catch (e) {
        setErrorTotalAll(e?.message || "Gagal memuat total penjualan");
        setTotalAll(0);
      } finally {
        setLoadingTotalAll(false);
      }
    })();
  }, []);
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-4">
      ...
      <Card>
        ...
        <div className="text-3xl font-bold">{fmtIDR(totalAll)}</div>
        {loadingTotalAll && (
          <div className="text-xs text-gray-500 mt-1">Memuat…</div>
        )}
        {errorTotalAll && (
          <div className="text-xs text-red-600 mt-1">Error: {errorTotalAll}</div>
        )}
      </Card>

      {/* Penjualan Hari Ini (tetap) */}
      <Card>
      ...
      {/* Kadaluarsa Dekat (opsional, tetap ada seperti sebelumnya) */}
      <Card>
          ...
          <div className="flex gap-2">
            <button
              className={`px-3 py-1 rounded ${
                span === 7 ? "bg-blue-600 text-white" : "bg-gray-100"
              }`}
              onClick={() => setSpan(7)}
            >
              7 Hari
            ...
            <button
              className={`px-3 py-1 rounded ${
                span === 30 ? "bg-blue-600 text-white" : "bg-gray-100"
              }`}
              onClick={() => setSpan(30)}
            >
              30 Hari
